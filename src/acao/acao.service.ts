  ...
  async getAcaoToday(nomeAcao: string): Promise<AcaoDto> {
    try {
      ...
      if (configA.dadosAcao && configA.dadosAcao.dataAcao) {
        ...
        logger.log(`dateLimite:${dateLimite} < dateNow:${dateNow}`);

        //
        //Case the last get of acao then get data of record
        if (dateNow < dateLimite) {
          logger.log(`Carregando os dados da acao salvo anteriormente`);
      ...
      logger.log(`Retornou dados na consulta na url: [${url}]`);
